...
__global__ void findBBnoise_gpu(int nSurf_BB,int nObs,int nTime,float dTau,float* span,float* omega,int* ccw,float* psi_offset,int* trip,float* rotate,float* translate,int* xsPtr,int* dsPtr,int* nsect,float* xyzcR,float* bbdata,int* nsect_t,int* nmach_t,int* naoa_t,int* nre_t,int* aPtr,int* mPtr,int* rPtr,int* sPtr,int* dPtr,float* sect_t,float* mach_t,float* aoa_t,float* re_t,float* bbdata_t,int t_del,float* vInf,float* rObs, float* tObs,float* ff,float a0,float* BBspl,float* BBoaspl);

__device__ void core_g(float rpnorm,float theta,float phi,float c,float lsect,float alpha,float u, float nu, float a0,int trip, float del_s,float del_p,float dis_s,float dis_p,int isect,int itime,int iobs,float* output_tmp,int nTime,int nsect,int isurf,float* ff);

__device__ void amin_g(float* a, float* amin, int n);
__device__ void amax_g(float* a, float* amax, int n);
__device__ void bmin_g(float* b, float* bmin, int n);
__device__ void bmax_g(float* b, float* bmax, int n);
__device__ float g5comp_g(float hdstar,float eta);
__device__ float pow2(float a,float b);

__device__ const int nf2 = 42;

__device__ float interp2d_g(float* xyzcR,int* xsPtr,int isurf,int isect,float* sect_t,int* sPtr,int is1,int is2,float mach,float* mach_t,int* mPtr,int im1,int im2,float* re,int ptr2,float* re_t,int* rPtr,int ir1,int ir2,int* dPtr,int* nmach_t,int* nre_t,int* naoa_t,float* bbdata_t,int id,int i);
...
float l1,theta,wa1,alpha_eff,cl_alpha0,alpha_cl0,cl_alpha,alpha_eff1,delta_0,deltas_0;
...
int iobs = blockIdx.x*blockDim.x + threadIdx.x;  // oberver
...
while (iobs<nObs)    
{
  ...
  for (int isurf=0; isurf<nSurf_BB; isurf++)
  {
	...
	c = xyzcR[xsPtr[isurf] + 4*isect + 3] * span[isurf];
	    ...
	    if( ccw[isurf]==0) x0[1] = -x0[1];

	    psi = (tObs[itime] + psi_offset[isurf]*M_PI/180);
	    ...
	    x1[1] =  x0[1];
	    x1[2] = -x0[1]*sin(thy) + x0[2]*cos(thy);
	    x1[1] =  x0[1]*cos(thy) + x0[2]*sin(thy);
	...
	for (int itime=0; itime<nTime; itime ++)
	{
	    ...
	    l1 = sqrt(pow2(v1[0],2) + pow2(v1[1],2) + pow2(v1[2],2));
...
////////////////// HARDCODED////////////
//	   u_ms[ptr2] = 145.05*1.75*xyzcR[xsPtr[isurf]+4*isect];

////////////////////////////////////////////////////
	    mach = u_ms[ptr2]/a0; 
	    re[ptr2] = u_ms[ptr2]*c/nu;
...
//if(isect==5&&itime==10)	
		cl[i] = interp2d_g(xyzcR,xsPtr,isurf,isect,sect_t,sPtr,is1,is2,mach,mach_t,mPtr,im1,im2,re,ptr2,re_t,rPtr,ir1,ir2,dPtr,nmach_t,nre_t,naoa_t,bbdata_t,0,i);
	    ...
	    // cl_alpha (per deg)
	    cl_alpha = (cl[a2] - cl[a1])/(aoa_t[aPtr[isurf]+a2] - aoa_t[aPtr[isurf]+a1]);
...
//if(isect==0&&itime==0) printf("%f %f %f %f %d\n",cl[a1],cl[a2],aoa_t[aPtr[isurf]+a2], aoa_t[aPtr[isurf]+a1],a2);
//aoa_t[aPtr[isurf]+a2],aoa_t[aPtr[isurf]+a1],wa1,a1);
	...
	// multipy by r/R**2 to adjust for Vref change assuming CFD CP uses reference tip Mach number
	    cn = bbdata[dsPtr[isurf] + itime*nsect[isurf]*7 + isect*7 + 4]/ pow2(xyzcR[xsPtr[isurf] + 4*isect],2);
	    ct = bbdata[dsPtr[isurf] + itime*nsect[isurf]*7 + isect*7 + 5]/ pow2(xyzcR[xsPtr[isurf] + 4*isect],2);
		...
		alpha_eff1 = (clsec-cl_alpha0)/cl_alpha;
...
//if(itime==0) printf("isect:%d alpha_eff:%f alpha_cl0:%f alpha:%f\n",isect,alpha_eff,alpha_cl0,alpha_eff+alpha_cl0); 
...
//printf("tdel=%d\n",t_del);
		...
		for (int jj=0; jj<2; jj++)
		{
		    ...
		    for (int i=0; i<4; i++)
		    {
		        bl_temp[jj*4+i] = interp2d_g(xyzcR,xsPtr,isurf,isect,sect_t,sPtr,is1,is2,mach,mach_t,mPtr,im1,im2,re,ptr2,re_t,rPtr,ir1,ir2,dPtr,nmach_t,nre_t,naoa_t,bbdata_t,i+3,iaoa);	    
		    }
		}
		for (int i=0; i<4; i++)
		   {
...
//if(itime==0&&isect==nsect[isurf]-2) printf("bl=%f", bl[isect*nTime*4+itime*4+i]);
		...
		delta_0 = c*pow2(10.0,(1.6569-0.9045*log10(re[ptr2])+0.0596*pow2(log10(re[ptr2]),2.0)));
		deltas_0 = c*pow2(10.0,(3.0187-1.5397*log10(re[ptr2])+0.1059*pow2(log10(re[ptr2]),2.0)));

		bl[isect*nTime*4+itime*4+1] = delta_0*pow2(10.0,(-0.04175*alpha[ptr2]+0.00106*pow2(alpha[ptr2],2.0)));
		bl[isect*nTime*4+itime*4+3]= deltas_0*pow2(10.0,(-0.0432*alpha[ptr2]+0.00113*pow2(alpha[ptr2],2.0)));


		if((alpha[ptr2])<=7.5) bl[isect*nTime*4+itime*4] = delta_0*pow2(10.0,(0.03114*alpha[ptr2]));
		else if((alpha[ptr2])>12.5) bl[isect*nTime*4+itime*4] = delta_0*12.0*pow2(10.0,0.0258*alpha[ptr2]);
		else bl[isect*nTime*4+itime*4] = delta_0*0.0303*pow2(10.0,0.2336*alpha[ptr2]);



		if((alpha[ptr2])<=7.5) bl[isect*nTime*4+itime*4+2] = deltas_0*pow2(10.0,(0.0679*alpha[ptr2]));
		else if((alpha[ptr2])>12.5) bl[isect*nTime*4+itime*4+2] = deltas_0*52.42*pow2(10.0,0.0258*alpha[ptr2]);
		else bl[isect*nTime*4+itime*4+2] = deltas_0*0.0162*pow2(10.0,0.3066*alpha[ptr2]);
...
//if(isect==nsect[isurf]-2)   printf("chord %f %f %f %d %d %d\n",c,xyzcR[xsPtr[isurf] + 4*isect + 3],span[isurf],xsPtr[isurf],isect,isurf);
		...
		for (int i=0; i<3; i++) 
		{
		    trans[1][i] = xyzt[kp[isect]*nTime*3 + itime*3+i] - xyzt[km[isect]*nTime*3 + itime*3+i];
		    tempr += pow2(trans[1][i],2);
		}
		...
		if (ccw[isurf] == 1)
		{
		    ...
		    tempr = sqrt(pow2(trans[2][0],2) + pow2(trans[2][1],2) + pow2(trans[2][2],2));
...
// {
//   for(int vv=0;vv<3;vv++) printf("%f ",trans[uu][vv]);
// }
// printf("\n");
// printf("%f %f %f %f %f %f\n",rp[0],rp[1],rp[2],rp2[0],rp2[1],rp2[2]);
//}
//if(itime==20) printf(" %d %f %f %f %f %f %f %f %f %f %d \n  %f %f %f %f %d\n",isect,rpnorm,theta,phi,c,lsect,alpha[ptr2],u_ms[ptr2],nu,a0,trip[isurf],bl[ptr3],bl[ptr3+1],bl[ptr3+2],bl[ptr3+3],nsect[isurf]);
//if(itime==0) printf(" %d %f %f %f %f %f %f %f %f %f\n",isect,rpnorm,theta,phi,c,lsect,alpha[ptr2],u_ms[ptr2],nu,a0);


//if(itime==0&&isect==nsect[isurf]-2) 
		core_g(rpnorm,theta,phi,c,lsect,alpha[ptr2],u_ms[ptr2],nu,a0,trip[isurf],bl[ptr3],bl[ptr3+1],bl[ptr3+2],bl[ptr3+3],isect,itime,iobs,output_tmp,nTime,nsect[isurf],isurf,ff);	



//printf("%d %f\n",isect*nTime + itime,output_tmp[0]);
	    } // itime
...
//for(int iw=0; iw<nf2; iw++) printf("%f %f %f %f %f %f %f\n",output_tmp[iw*7],output_tmp[iw*7+1],output_tmp[iw*7+2],output_tmp[iw*7+3],output_tmp[iw*7+4],output_tmp[iw*7+5],output_tmp[iw*7+6]);
  ...
  for (int i=0; i<nf2; i++)
  {
   	for (int j=0; j<7; j++)
	{
	    for (int itime=0; itime<nTime; itime++)
	    {
	    	BBspl[iobs*nf2*7+i*7+j] = 10.0*log10(pow2(10.0,(BBspl[iobs*nf2*7+i*7+j]/10.0)) + pow2(10.0,(output_tmp[iobs*nTime*nf2*7 + itime*nf2*7 + i*7 + j]/10.0))); 
	    }
	    BBspl[iobs*nf2*7+i*7+j] = 10.0*log10(pow2(10.0,(BBspl[iobs*nf2*7+i*7+j]/10.0))/(float)nTime);
	}
//printf("%f %f\n",BBoaspl[iobs],BBspl[iobs*nf2*7+i*7]);
	BBoaspl[iobs] = 10.0*log10(pow2(10.0,(BBoaspl[iobs]/10.0)) + pow2(10.0,(BBspl[iobs*nf2*7+i*7]/10.0)));
  }

  iobs += gridDim.x*blockDim.x;
  __syncthreads();
} // while iobs<nobs
...
//printf("%d %f %f %f %f %f %f %f %f %f %d %f %f %f %f %d %d %d %d %d %d\n",isect,rpnorm,theta,phi,c,lsect,alpha,u,nu,a0,trip,del_s,del_p,dis_s,dis_p,itime,iobs,nTime,nf2,nsect,isurf);
//printf("\n");
float mach,r_c,m_c,d_h,d_l;
int optr;

//float* pmin = (float*)malloc(nf2*sizeof(float));
//float* pmax = (float*)malloc(nf2*sizeof(float));
//float* smin = (float*)malloc(nf2*sizeof(float));
//float* smax = (float*)malloc(nf2*sizeof(float));
//float* aas = (float*)malloc(nf2*sizeof(float));
//float* aap = (float*)malloc(nf2*sizeof(float));
//float* b = (float*)malloc(nf2*sizeof(float));
//float* bmin = (float*)malloc(nf2*sizeof(float));
//float* bmax = (float*)malloc(nf2*sizeof(float));
//float* bb = (float*)malloc(nf2*sizeof(float));
//float* spl_a = (float*)malloc(nf2*sizeof(float));
//float* spl_p = (float*)malloc(nf2*sizeof(float));
//float* spl_s = (float*)malloc(nf2*sizeof(float));
//float* aminb = (float*)malloc(nf2*sizeof(float));
//float* amaxb = (float*)malloc(nf2*sizeof(float));
//float* anew2 = (float*)malloc(nf2*sizeof(float));
...
d_h = (2.0*pow2(sin(theta/2.0),2) * pow2(sin(phi),2)) / ((1.0+mach*cos(theta))*pow2(1.0+(mach-m_c)*cos(theta),2));

d_l = (pow2(sin(theta),2) * pow2(sin(phi),2)) / (pow2(1.0+mach*cos(theta),4));
...
} else
{
	...
	{
		st_2 = st_1* pow2(10.0,(0.0054*pow2(alpha-1.33,2.0)));
	}
}
...
if( r_c < 9.52*pow2(10,4))
{
	a_0[0] = 0.57;
}else
{
	if( r_c > 8.57*pow2(10,5)) a_0[0] = 1.13;
	else a_0[0] = (-9.57*pow2(10,-13))*(pow2((r_c-(8.57*pow2(10,5))),2)) + 1.13;
}
...
{
	if(r_c > 8.57*pow2(10,5))
	{
		b_0[0] = 0.56;
	} else
	{
		b_0[0] = (-4.48*pow2(10,-13))*(pow2((r_c-(8.57*pow2(10,5))),2)) + 0.56;
	}
}
// compute function
...
if(r_c<2.47*pow2(10,5))
{
	k_1 = 4.31*log10(r_c) + 156.3;
} else
{
	if(r_c>8.0*pow2(10,5))
	{
		k_1 = 129.5;
	} else
	{
...
} else
{
	...
	{
		k_2 = k_1+sqrt(pow2(beta,2) - pow2(beta/gamm,2)*pow2(alpha-gamm_0,2)) + beta_0;
	}
}
...
for (int i=0; i<nf2; i++)
{	// pressure side
	spl_p[i] = 10.0*log10((dis_p*pow2(mach,5)*lsect*d_h)/(pow2(rpnorm,2.0))) + aap[i] + (k_1-3.0) + deltak_1;
	// suction side
	spl_s[i] = 10.0*log10((dis_s*pow2(mach,5)*lsect*d_h)/(pow2(rpnorm,2.0))) + aas[i] + (k_1-3.0);
//printf("%f %f %f %f %f %f %f %f %f %f %f %f\n",spl_p[i],spl_s[i],dis_p,dis_s,mach,lsect,d_h,rpnorm,aap[i],aas[i],k_1,deltak_1);
...
{
	r_c_new = 3.0*r_c;
	if(r_c_new < 9.52*pow2(10,4))
	{
		a_0[0] = 0.57;
	} else
	{
		if(r_c_new > 8.57*pow2(10,5)) a_0[0] = 1.13;
		else a_0[0] = -9.57*pow2(10.0,-13.0)*pow2(r_c_new-(8.57*pow2(10,5)),2) + 1.13;
	}

	if(r_c_new < 2.47*pow2(10,5))
	{
		k_1 = -4.1*log10(r_c_new) + 156.3;
	} else
	{
		if(r_c_new > 8*pow2(10,5)) k_1 = 128.5;
	...
	} else
	{
		...
		else k_2 = k_1 + sqrt(pow2(beta,2) - (pow2(beta/gamm,2))*(pow2(alpha-gamm_0,2))) + beta_0;
	}
	...
	amin_g(anew,aminb,nf2);
	amax_g(anew,amaxb,nf2);

	for(int i=0; i<nf2; i++)
	{
		anew2[i] = aminb[i] + a_r*(amaxb[i]-aminb[i]);
		spl_a[i] = 10.0*log10((dis_s*pow2(mach,5)*lsect*d_l)/(pow2(rpnorm,2))) + anew2[i] +k_2+10.0;
	}

//printf("spl_a noise-1\n");
//for(int i=0; i<nf2; i++) printf("%f %f %f %f %f %f %f %f\n",spl_a[i],dis_s,mach,lsect,d_l,rpnorm,anew2[i],k_2);	

} else
{
	for(int i=0; i<nf2; i++) spl_a[i] = 10.0*log10((dis_s*pow2(mach,5)*lsect*d_h)/(pow2(rpnorm,2))) + bb[i] +k_2+10.0;
...
for (int i=0; i<nf2; i++)
{
	output_tmp[optr + i*7 + 1] = 10.0*log10(pow2(10.0,output_tmp[optr+i*7+1]/10.0) + pow2(10.0,spl_a[i]/10.0));
	output_tmp[optr + i*7 + 2] = 10.0*log10(pow2(10.0,output_tmp[optr+i*7+2]/10.0) + pow2(10.0,spl_s[i]/10.0));
	output_tmp[optr + i*7 + 3] = 10.0*log10(pow2(10.0,output_tmp[optr+i*7+3]/10.0) + pow2(10.0,spl_p[i]/10.0));

	output_tmp[optr + i*7] = 10.0*log10(pow2(10.0,output_tmp[optr+i*7]/10.0) + pow2(10.0,spl_a[i]/10.0) + pow2(10.0,spl_p[i]/10.0) + pow2(10.0,spl_s[i]/10.0));
...
if(trip==0)
{
	if(r_c < 1.3*pow2(10,5))	stlprim = 0.18;
	else if(r_c>1.3*pow2(10,5) && r_c <= 4*pow2(10,5)) stlprim = 0.001756*pow2(r_c,0.3931);
	else stlprim = 0.28;

	stpkprm = pow2(10,(-0.04*alpha))*stlprim;

	if(alpha<=3.0) r_c0 = pow2(10,(0.215*alpha+4.978));
	else r_c0 = pow2(10,(0.120*alpha+5.263));
	...
	else if(d<=1.7579) g2 = -114.052*pow2(log10(d),2);
	else if(d<=3.0889) g2 = -65.188*log10(d)+9.125;
	else g2 = -77.852*log10(d)+15.328;

	g3 = 171.04 - 3.03*alpha;
	scal = 10.0*log10(del_p*pow2(mach,5)*d_h*lsect/(pow2(rpnorm,2)));
	...
	for (int i=0; i<nf2; i++)
	{
		...
		else if(e[i]<=1.17) g1[i] = -5.076+sqrt(2.484-506.25*(pow2(log10(e[i]),2)));
		else if(e[i]<=1.674) g1[i] = -98.409*log10(e[i])+2.0;
		else g1[i] = -39.80*log10(e[i])-11.12;

		spl_lam = g1[i] +g2+g3+scal;
	
		output_tmp[optr + i*7 + 4] = 10.0*log10(pow2(10.0,output_tmp[optr+i*7+4]/10.0) + pow2(10.0,spl_lam/10.0));
		output_tmp[optr + i*7] = 10.0*log10(pow2(10.0,output_tmp[optr+i*7]/10.0) + pow2(10.0,spl_lam/10.0));
	}	
}
...
if(hdstar >= 0.2) stpeak = aterm/(1.0+0.235*dstarh-0.0132*pow2(dstarh,2));
...
{
	hdstarl = hdstar;
	g514 = g5comp_g(hdstarl,eta[i]);

	hdstarp = 6.724*(pow2(hdstar,2)) - 4.019*hdstar + 1.107;

	g50 = g5comp_g(hdstarp,eta[i]);

	g5 = g50 + 0.0714*psi*(g514-g50);

	if(g5>0) g5=0.0;

	f4temp = g5comp_g(0.25,eta[i]);

	if(g5>f4temp) g5=f4temp;

	scal = 10.0*log10(pow2(mach,5.5)*h*d_h*lsect/(pow2(rpnorm,2)));

	spl_blunt = g4+g5+scal;
	output_tmp[optr + i*7 + 5] = 10.0*log10(pow2(10.0,output_tmp[optr+i*7+5]/10.0) + pow2(10.0,spl_blunt/10.0));
	output_tmp[optr + i*7] = 10.0*log10(pow2(10.0,output_tmp[optr+i*7]/10.0) + pow2(10.0,spl_blunt/10.0));
...
// TVF (tip noise)
float mm,um,term,tip_l,stpp[nf2];
if(isect == nsect-2)
{
	int iround = 0;
	float alprat = 1.0; // correction
	...
	term = mach*mach*pow2(mm,3)*pow2(tip_l,2)*d_h/(pow2(rpnorm,2));
...
//printf("%f %f %f %f %f\n\n",scal,term,um,alpha,mach);
	for (int i=0; i<nf2; i++)
	{
		...
		spl_tip = 126.0-30.5*pow2((log10(stpp[i])+0.3),2) + scal;
		output_tmp[optr + i*7 + 6] = 10.0*log10(pow2(10.0,output_tmp[optr+i*7+6]/10.0) + pow2(10.0,spl_tip/10.0));
		output_tmp[optr + i*7] = 10.0*log10(pow2(10.0,output_tmp[optr+i*7]/10.0) + pow2(10.0,spl_tip/10.0));
...
for (int i=0; i<n; i++)
{
	if(a[i] < 0.204)
	{
		amin[i] = sqrt(67.552 - 886.788*pow2(a[i],2)) - 8.219;
	} else
	{
		if(a[i] > 0.244)
		{
			amin[i] = -142.795*pow2(a[i],3) + 103.656*pow2(a[i],2) - 57.757*a[i] + 6.006;
		} else
		{
...
for (int i=0; i<n; i++)
{
	if(a[i] < 0.13)
	{
		amax[i] = sqrt(67.552 - 886.788*pow2(a[i],2)) - 8.219;
	} else
	{
		if(a[i] > 0.321)
		{
			amax[i] = -4.669*pow2(a[i],3) + 3.491*pow2(a[i],2) - 16.699*a[i] + 1.149;
		} else
		{
...
for (int i=0; i<n; i++)
{
	if(b[i] < 0.13)
	{
		bmin[i] = sqrt(16.888 - 886.788*pow2(b[i],2)) - 4.109;
	} else
	{
		if(b[i] > 0.145)
		{
			bmin[i] = -817.810*pow2(b[i],3) + 355.21*pow2(b[i],2) - 135.024*b[i] + 10.619;
		} else
		{
...
for (int i=0; i<n; i++)
{
	if(b[i] < 0.1)
	{
		bmax[i] = sqrt(16.888 - 886.788*pow2(b[i],2)) - 4.109;
	} else
	{
		if(b[i] > 0.187)
		{
			bmax[i] = -80.541*pow2(b[i],3) + 44.174*pow2(b[i],2) - 39.381*b[i] + 2.344;
		} else
		{
...
eta0 = -sqrt((m*m*(pow2(mu,4)))/(6.25+m*m*mu*mu));

k = 2.5*sqrt(1.0-(pow2(eta0/mu,2))) -2.5-m*eta0;

if(eta<=eta0) g5=m*eta+k;
else if(eta<=0.0) g5=2.5*sqrt(1.0-pow2(eta/mu,2))-2.5;
else if(eta<=0.03616) g5=sqrt(1.5625-1194.99*pow2(eta,2))-1.25;
